...
usage() {
  echo "Usage: $0 --controller <name> --scenario <6hdd|4ssd>"
  exit 1
}
...
log "Controller label: $CONTROLLER"
...
# ------------------------------------------------------------
{
  ...
  echo
  echo "=== LSPCI STORAGE ==="
  lspci -nn | grep -Ei 'sas|scsi|sata|asmedia|lsi|broadcom'
  echo
  echo "=== LSPCI VERBOSE STORAGE ==="
  lspci -nnvv | grep -A25 -Ei 'Serial Attached SCSI|SATA controller|ASMedia|LSI|Broadcom'
  echo
  echo "=== LSBLK ==="
...
# ------------------------------------------------------------
# Define disk sets
# EDIT THESE IDS FOR YOUR REAL SSDS BEFORE RUNNING 4ssd
# ------------------------------------------------------------
...
SSD3="/dev/disk/by-id/ata-REPLACE_ME_SSD3"
SSD4="/dev/disk/by-id/ata-REPLACE_ME_SSD4"

if [[ "$SCENARIO" == "6hdd" ]]; then
  DISKS=("$HDD1" "$HDD2" "$HDD3" "$HDD4" "$HDD5" "$HDD6")
elif [[ "$SCENARIO" == "4ssd" ]]; then
  DISKS=("$SSD1" "$SSD2" "$SSD3" "$SSD4")
fi
...
log "Selected disks:"
printf '%s\n' "${DISKS[@]}" | tee -a "${OUTDIR}/run.log"

# ------------------------------------------------------------
# SMART snapshots for selected disks
# ------------------------------------------------------------
for d in "${DISKS[@]}"; do
  base=$(basename "$d")
...
# controller-revealing concurrency sweep
