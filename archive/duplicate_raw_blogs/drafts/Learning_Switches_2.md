# Access — it always starts here: web, serial, telnet, ssh
⸻

Access — it always starts here: web, serial, telnet, ssh

(The four doors into your switch — pros, cons, and how to use them safely)

There are two kinds of network gear users: those who think “I’ll just use the web GUI” and those who immediately reach for a serial cable. I’m learning to be a messy hybrid — I want the convenience of a browser and the precision of a CLI, plus the confidence that I can always reach the hardware even if the network breaks. A managed switch like the TP-Link SG3428 gives you four ways in: web, serial, telnet, and SSH. Here’s how I tried them, what worked, and the gotchas I’d warn you about.
...
Commands / Steps
	•	Assign laptop IP on same subnet (example):
...
Screenshot idea: web UI dashboard showing firmware and interface list.

Security quick wins (do these ASAP after first login)
	•	Change the default password to a long, unique passphrase (you already did — good).
...
Screenshot idea: terminal with boot output and prompt.
...
(See the cheat sheet for the TP-Link syntax we used.)
...
Screenshot idea: telnet session with the banner and show vlan output.
...
Screenshot idea: SSH login and show system-info output.
...
(Suggestion for a screenshot or diagram: simple network diagram showing management VLAN separated from guest/family VLAN.)
...
(Full cheat sheet is appended to the blog post as an appendix — or link to your Anki deck.)
